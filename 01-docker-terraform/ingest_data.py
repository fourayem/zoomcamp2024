...
def main(params):
    ...
    df = pd.read_csv(f'{csv_name}', chunksize=100000, iterator=True, compression='gzip')
    ...
    print(
        pd.io.sql.get_schema(df_chunk_header, name=table_name, con=engine)
    )

    df_chunk_header.to_sql(name=table_name, con=engine, if_exists='replace', index=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--table_name', help='table name of where ny taxi data will be loaded')
